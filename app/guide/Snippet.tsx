...
export default function Snippet({ code, label }: { code: string; label: string }) {
  ...
  return (
    <div style={{ border: '1px solid var(--edge)', background: '#04060a', margin: '12px 0 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        padding: '7px 12px', borderBottom: '1px solid var(--edge)' }}>
      ...
      <pre style={{ margin: 0, padding: '12px 14px', overflowX: 'auto', fontSize: 12.5, lineHeight: 1.6, color: 'var(--ink)' }}>
        <code>{code}</code>
      </pre>
    </div>
  )
